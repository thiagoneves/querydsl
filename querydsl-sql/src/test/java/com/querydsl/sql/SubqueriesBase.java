...
public class SubqueriesBase extends AbstractBaseTest {
    ...
    public void SubQuery_Alias() {
        query().from(query().from(employee).select(employee.all()).as(employee2)).select(employee2.all()).list();
    }
    ...
    public void SubQuery_All() {
        query().from(employee).where(employee.id.gtAll(
                query().from(employee2).select(employee2.id))).count();
    }
    ...
    public void SubQuery_InnerJoin() {
        ...
        query().from(employee).innerJoin(sq, sqEmp).on(sqEmp.id.eq(employee.id)).select(employee.id).list();
    ...
    public void SubQuery_LeftJoin() {
        SubQueryExpression<Integer> sq = query().from(employee2).select(employee2.id);
        QEmployee sqEmp = new QEmployee("sq");
        query().from(employee).leftJoin(sq, sqEmp).on(sqEmp.id.eq(employee.id)).select(employee.id).list();
    ...
    public void SubQuery_Params() {
        ...
        assertEquals(1, query().from(subQuery.select(Wildcard.all)).count());
    }
    ...
    public void SubQuery_RightJoin() {
        SubQueryExpression<Integer> sq = query().from(employee2).select(employee2.id);
        QEmployee sqEmp = new QEmployee("sq");
        query().from(employee).rightJoin(sq, sqEmp).on(sqEmp.id.eq(employee.id)).select(employee.id).list();
    }

    @Test
    public void SubQuery_with_Alias() {
        List<Integer> ids1 = query().from(employee).select(employee.id).list();
        List<Integer> ids2 = query().from(query().from(employee).select(employee.id), employee).select(employee.id).list();
    ...
    public void SubQuery_with_Alias2() {
        List<Integer> ids1 = query().from(employee).select(employee.id).list();
        List<Integer> ids2 = query().from(query().from(employee).select(employee.id).as(employee)).select(employee.id).list();
        assertEquals(ids1, ids2);
    }
