...
public class SelectWindowFunctionsBase extends AbstractBaseTest {
    ...
    public void WindowFunctions() {
        ...
        for (WindowOver<?> wo : exprs) {
            query().from(survey).select(wo.over().partitionBy(survey.name).orderBy(survey.id)).list();
        }
    }
    ...
    public void WindowFunctions_Manual_Paging() {
        ...
        for (Tuple row : query().from(employee).select(employee.firstname, employee.lastname, rowNumber).list()) {
        ...
        for (Object[] row : query().from(sub.as(subAlias)).select(all).list()) {
        ...
        for (Object[] row : query().from(sub2.as(subAlias2)).select(all).list()) {
            System.out.println(Arrays.asList(row));
        }
        System.out.println();
        ...
        for (Tuple row : query().from(sub3.as(employee2)).select(employee2.firstname, employee2.lastname).list()) {
            System.out.println(Arrays.asList(row));
        }
    }
    ...
    public void WindowFunctions_Keep() {
        ...
        for (WindowOver<?> wo : exprs) {
            query().from(survey).select(wo.keepFirst().orderBy(survey.id)).list();
        }
    }
    ...
    public void WindowFunctions_Oracle() {
        ...
        for (WindowOver<?> wo : exprs) {
            query().from(survey).select(wo.over().partitionBy(survey.name)).list();
        }
    }
    ...
    public void WindowFunctions_Over() {
        ...
        query().from(employee).select(
                employee.id,
                SQLExpressions.sum(employee.salary).over()).list();
    }
    ...
    public void WindowFunctions_PartitionBy() {
        ...
        query().from(employee).select(
                ...
                SQLExpressions.sum(employee.salary).over()
                    .partitionBy(employee.superiorId)).list();
    }
    ...
    public void WindowFunctions_OrderBy() {
        ...
        query().from(employee).select(
                ...
                SQLExpressions.sum(employee.salary).over()
                    ...
                    .orderBy(employee.datefield)).list();
    }
    ...
    public void WindowFunctions_UnboundedRows() {
        ...
        query().from(employee).select(
                ...
                SQLExpressions.sum(employee.salary).over()
                    ...
                    .rows().between().unboundedPreceding().currentRow()).list();
    }
    ...
    public void WindowFunctions_Qualify() {
        ...
        teradataQuery().from(employee)
               ...
               .select(employee.id,SQLExpressions.sum(employee.salary).over()).list();
