...
public class SpatialBase extends AbstractBaseTest {
    ...
    public void SpatialRefSys() {
        ...
        query().from(spatialRefSys).select(spatialRefSys).list();
    }
    ...
    public void GeometryType() {
        List<Tuple> results = query().from(shapes).select(shapes.geometry, shapes.geometry.geometryType()).list();
    ...
    @Test
    public void AsText() {
        List<Tuple> results = query().from(shapes).select(shapes.geometry, shapes.geometry.asText()).list();
        assertFalse(results.isEmpty());
        for (Tuple row : results) {
    ...
    public void Point_X_Y() {
        ...
        List<Tuple> results = withPoints().select(point, point.x(), point.y()).list();
        assertFalse(results.isEmpty());
        for (Tuple row : results) {
    ...
    public void Point_Distance() {
        ...
        for (Tuple tuple : query().from(shapes1, shapes2)
                    ...
                    .select(shapes1.geometry.asPoint(),
                          ...
                          shapes1.geometry.distance(shapes2.geometry)).list()) {
    ...
    public void Point_Instances() {
        List<Shapes> results = withPoints().select(shapes).list();
    ...
    public void LineString_Instances() {
        List<Geometry> results = withLineStrings().select(shapes.geometry).list();
        assertFalse(results.isEmpty());
    ...
    public void Polygon_Instances() {
        List<Geometry> results = withPolygons().select(shapes.geometry).list();
        assertFalse(results.isEmpty());
        for (Geometry row : results) {
            assertNotNull(row);
    ...
    public void MultiPoint_Instances() {
        List<Geometry> results = withMultipoints().select(shapes.geometry).list();
        assertFalse(results.isEmpty());
        for (Geometry row : results) {
            assertNotNull(row);
    ...
    public void MultiLineString_Instances() {
        List<Geometry> results = withMultiLineStrings().select(shapes.geometry).list();
        assertFalse(results.isEmpty());
        for (Geometry row : results) {
            assertNotNull(row);
    ...
    public void MultiPolygon_Instances() {
        List<Geometry> results = withMultiPolygons().select(shapes.geometry).list();
        assertFalse(results.isEmpty());
        for (Geometry row : results) {
            assertNotNull(row);
    ...
    public void Point_Methods() {
        ...
        for (Expression<?> expr : expressions) {
            ...
            for (Object row : withPoints().select(expr).list()) {
                if (row == null && !logged) {
    ...
    public void Point_Methods2() {
        ...
        for (Expression<?> expr : expressions) {
            ...
            for (Object row : query().from(shapes1, shapes2)
                    .where(shapes1.id.loe(5), shapes2.id.loe(5)).select(expr).list()) {
                if (row == null && !logged) {
                    System.err.println(expr.toString());
                    logged = true;
                }
    ...
    public void LineString_Methods() {
        ...
        for (Expression<?> expr : expressions) {
            boolean logged = false;
            for (Object row : withLineStrings().select(expr).list()) {
                if (row == null && !logged) {
                    System.err.println(expr.toString());
                    logged = true;
                }
    ...
    public void Polygon_Methods() {
        ...
        for (Expression<?> expr : expressions) {
            boolean logged = false;
            for (Object row : withPolygons().select(expr).list()) {
                if (row == null && !logged) {
                    System.err.println(expr.toString());
                    logged = true;
                }
    ...
    public void MultiPoint_Methods() {
        ...
        for (Expression<?> expr : expressions) {
            boolean logged = false;
            for (Object row : withMultipoints().select(expr).list()) {
                if (row == null && !logged) {
                    System.err.println(expr.toString());
                    logged = true;
                }
    ...
    public void MultiLineString_Methods() {
        ...
        for (Expression<?> expr : expressions) {
            boolean logged = false;
            for (Object row : withMultiLineStrings().select(expr).list()) {
                if (row == null && !logged) {
                    System.err.println(expr.toString());
                    logged = true;
                }
    ...
    public void MultiPolygon_Methods() {
        ...
        for (Expression<?> expr : expressions) {
            boolean logged = false;
            for (Object row : withMultiPolygons().select(expr).list()) {
                if (row == null && !logged) {
                    System.err.println(expr.toString());
                    logged = true;
                }
