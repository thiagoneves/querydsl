...
public class OrderingTest extends AbstractJDOTest {
    ...
    public void Order_Desc() {
        List<String> namesDesc = query().from(product).orderBy(
                product.name.desc()).select(product.name).list();
    ...
    public void TabularResults() {
        List<Tuple> rows = query().from(product).orderBy(product.name.asc())
                .select(product.name, product.description).list();
    ...
    public void Limit_Order_Asc() {
        assertEquals(Arrays.asList("A0", "A1"), 
            query().from(product).orderBy(product.name.asc()).limit(2).select(product.name).list());
    }
    
    @Test
    public void Limit_Order_Desc() {
        assertEquals(Arrays.asList("C9", "C8"), 
            query().from(product).orderBy(product.name.desc()).limit(2).select(product.name).list());
    }
    
    public void Limit_and_Offset() {
        assertEquals(Arrays.asList("A2", "A3", "A4"), 
            query().from(product).orderBy(product.name.asc()).offset(2).limit(3).select(product.name).list());
    }

    @Test
    public void SearchResults() {
        QueryResults<String> results = query().from(product).orderBy(
                product.name.asc()).limit(2).select(product.name).listResults();
